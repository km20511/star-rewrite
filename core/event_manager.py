...
class EventManager:
    ...
    def register_effect(self, game_manager: GameManager, effect_obj: Effect):
        ...
        readonlys: Dict[str, Any] = (
            game_manager.get_readable_static_table()
            | game_manager.deck.get_readable_static_table()
            | game_manager.inventory.get_readable_static_table()
        ...
        match (effect_data.event_type):
            case EventType.OnShown | EventType.OnEntered | EventType.OnPurchased | EventType.OnDestroyed:
                self.__listeners_table[effect_data.event_type].append(EventHandler1[Card](
                    effect_obj.owner,
                    lambda gm, card: inner_func(gm, target=card)
                ))
            case EventType.OnUsed:
                self.__on_item_used_listeners.append(EventHandler1[Item](
                    effect_obj.owner,
                    lambda gm, card: inner_func(gm, target=card)
                ))
    ...
    def on_card_shown(
        self, game_manager: GameManager, target: Card, immediate: bool = False
    ):
    ...
    def on_item_used(
        self, game_manager: GameManager, target: Item, immediate: bool = False
    ):
        ...
        if immediate:
            ...
            for listener in self.__on_player_stat_changed_listeners:
                self.__event_queue.append(
                    lambda: listener.invoke(game_manager, stat_type, previous, current)
                )

    def on_turn_begin(
        self, game_manager: GameManager, current_turn: int, immediate: bool = False
    ):
        ...
        if immediate:
            ...
            for listener in self.__on_turn_begin_listeners:
                self.__event_queue.append(
                    lambda: listener.invoke(game_manager, current_turn)
                )
    ...
    def on_card_cost_changed(
        ...
        if immediate:
            ...
            for listener in self.__on_card_cost_changed_listeners:
                self.__event_queue.append(
                    lambda: listener.invoke(game_manager, target, previous, current)
                )
    ...
    def on_calculate_card_cost(
        self, game_manager: GameManager, immediate: bool = False
    ):
        if immediate:
